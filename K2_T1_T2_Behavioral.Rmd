...
# K2_T2_BehavioralData_20200728.csv
...
b_both # this will show up in the rendered html
```
...
# t1 desc stats, include norm stats
...
# t2 desc stats, include norm stats
...
### By age
```{r ctopp color scatter by timept, echo=F, results='hide', fig.width=10, fig.height=5}

# t1 ctopp color scatter (x = age, y = raw)
p_cc_1 <- b1_clean %>% 
  drop_na(t1.ctopp_c_raw) %>% 
  ggplot(aes(x = t1.age, y = t1.ctopp_c_raw, color = grade)) + 
  geom_point() +
...
p_cc_2 <- b2_clean %>% 
  ...
  ggplot(aes(x = t2.age, y = t2.ctopp_c_raw, color = grade)) + 
  geom_point() +
  scale_x_continuous(breaks = seq(5, 9, by = 0.5)) +
...
### Both time points
```{r ctopp color t1-t2, echo=FALSE, fig.width=8, fig.height=5}

# t1-t2 ctopp color scatterplot of raw scores
p_cc_5 <- ggplot(b_both, aes(x = t1.ctopp_c_raw, y = t2.ctopp_c_raw)) + 
...
### By age
```{r ctopp letter scatter by timept, echo=F, results='hide', fig.width=10, fig.height=5}

# t1 ctopp letter scatter (x = age, y = raw)
p_cl_1 <- b1_clean %>% 
  drop_na(t1.ctopp_l_raw) %>% 
  ggplot(aes(x = t1.age, y = t1.ctopp_l_raw, color = grade)) + 
  geom_point() +
  scale_x_continuous(breaks = seq(5, 9, by = 0.5)) +
...
p_cl_2 <- b2_clean %>% 
  ...
  ggplot(aes(x = t2.age, y = t2.ctopp_l_raw, color = grade)) + 
  geom_point() +
  scale_x_continuous(breaks = seq(5, 9, by = 0.5)) +
...
### Both time points
```{r ctopp letter t1-t2, echo=FALSE, fig.width=8, fig.height=5}
...
# just testing this for now...
lm_cc <- lm(t2.ctopp_l_raw ~ t1.ctopp_l_raw, data = b_both)
print(lm_cc)
print(summary(lm_cc))
...
```{r elision plots, echo=F, fig.width=10, fig.height=5}

# t2 ctopp elision scatter (x = age, y = raw)
p_ce_1 <- b2_clean %>% 
  ggplot(aes(x = t2.age, y = t2.elision_raw, color = grade)) + 
  geom_point() +
  scale_x_continuous(breaks = seq(5, 9, by=0.5)) + 
